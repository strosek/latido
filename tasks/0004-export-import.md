...
- Export: serialize the full app state (tasks, sessions, notes, settings) to a JSON file and
  trigger a download (e.g. `ultradiandrift-export-YYYY-MM-DD.json`).
...
- Importing a malformed or non-ultradiandrift file is rejected without changing current data.
